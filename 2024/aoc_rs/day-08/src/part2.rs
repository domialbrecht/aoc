        ...
        .flat_map(|chunk| {
            chunk
                ...
                .flat_map(|stats| {
                    ...
                    let first_results: Vec<_> = successors(Some(stats[0].0), |pos| {
                        let new_pos = pos + diff;
                        if x_bounds.contains(&new_pos.x) && y_bounds.contains(&new_pos.y) {
                            Some(new_pos)
                        } else {
                            None
                        }
                    ...
                    .collect();
                    let second_results: Vec<_> = successors(Some(stats[1].0), |pos| {
                        let new_pos = pos - diff;
                        if x_bounds.contains(&new_pos.x) && y_bounds.contains(&new_pos.y) {
                            Some(new_pos)
                        } else {
